// TAG2 코드: motion interrupt 발생 시 A1/A2/A3/A4를 순차 ranging
...
void initOLED() {
  ...
  display.println("TAG2 OLED READY");
  display.display();
}
...
void setupAwakeModeAfterMotionWake() {
  ...
  Serial.println("TAG2 AWAKE by motion");
}
...
void doOneRangingAttemptIfPossible() {
  ...
  Serial.print("TAG2 HYBRID SEND POLL to A");
  ...
  if (!sendFrame(tx_poll_msg, sizeof(tx_poll_msg))) {
    ...
    if (!waitForResp(RESP_TIMEOUT_MS)) {
      ...
      if (!sendFrame(tx_final_msg, sizeof(tx_final_msg))) {
        Serial.println("TAG2 FINAL fail");
        ...
        if (includeBatteryThisFinal) {
          ...
          Serial.print("TAG2 BAT SENT ONCE=");
          Serial.print(wakeBattery.batteryVoltage, 2);
          Serial.print("V PCT=");
          Serial.print(wakeBattery.percent);
          Serial.println("%");
        } else {
          Serial.println("TAG2 BAT SKIP: already sent in this awake cycle");
        }
...
void setup() {
  ...
  delay(300);             // 0.3초 대기
  Serial.println("TAG2 UWB FINAL_MV ORDERED_ANCHOR v2 - FINAL_SIZE=" + String(sizeof(tx_final_msg)));
