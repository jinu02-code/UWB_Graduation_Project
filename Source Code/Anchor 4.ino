// Anchor 4 코드: TAG1/TAG2의 A4 대상 ranging 요청 처리
#include <SPI.h>
#include <dw3000.h>
#include <WiFi.h>
...
void sendToRaspberryPi(uint8_t tag_id,
                       ...
                       uint8_t seq,
                       double distance,
                       float avg_distance) {  // CHANGED: Tag 배터리 전압 수신 제거로 battery_voltage 인자 삭제
  ...
  // CHANGED: Tag에서 수신한 배터리 전압을 Raspberry Pi로 보내는 JSON 필드를 제거함.
  // 기존 거리/평균거리/Tag ID/Anchor ID/seq 전송 구조는 유지함.
...
void setup() {
  ...
  Serial.println("ANCHOR 1 RANGING v1 (TAG1/TAG2)");  // CHANGED: Tag 배터리 전압 수신 기능 제거 반영
  initWiFi();
  initDW3000();
  unlockAnchor();
}
...
void loop() {
  ...
  Serial.print(" m | avg: ");
  Serial.println(avg, 3);  // CHANGED: Tag 배터리 전압 Serial 출력 제거

  // Raspberry Pi/AWS 연동용 파싱 라인
  // 형식: DATA,TAG_ID,ANCHOR_ID,SEQ,DISTANCE_M,AVG_DISTANCE_M
  Serial.print("DATA,");
  Serial.print(tag_id);
  Serial.print(",");
  Serial.print(MY_ANCHOR_ID);
  ...
  unlockAnchor();
  sendToRaspberryPi(tag_id, MY_ANCHOR_ID, seq, distance, avg);  // CHANGED: battery_voltage 인자 제거
  delay(2);
}
